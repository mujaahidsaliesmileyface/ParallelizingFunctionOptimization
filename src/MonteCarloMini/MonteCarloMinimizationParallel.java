...
class MonteCarloMinimizationParallel{
    ...
    public static void main(String[] args)  {
        ...
        SearchParallel searchParallel = new SearchParallel(searches);

        ForkJoinPool fjPool = new ForkJoinPool() ;

        //start timer
    	tick();
        int min = fjPool.invoke(searchParallel) ;
        tock();
        //end timer
		...
		/* Results*/
        System.out.printf("Global minimum: %d at x=%.1f y=%.1f\n\n", min, terrain.getXcoord(searchParallel.minRow), terrain.getYcoord(searchParallel.minCol));
